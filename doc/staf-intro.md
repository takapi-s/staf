大量のCSVに対して、要約・分類・抽出などの「AIによる付加価値」を一括で載せたい。けれど、LLMの出力は形がブレやすく、列にきれいに落とし込むのは大変——STAF（Structured AI Flow）は、この現実的な悩みを解決するために生まれた、スキーマ準拠のAIエンリッチメント（付与・抽出・正規化）ツールです。  
現在は Windows 向け配布（10/11）を優先していますが、将来的に macOS / Linux への対応も視野に入れています。Tauri ベースのため移植性は確保済みです。
...
2. CSV を読み込む（プレビューで確認）  
3. 「Prompt（テンプレ）」を作成（行の値は `{{columnName}}` で差し込み）  
4. 「Output Columns（出力列）」を型/ネスト含めて定義（これがJSONスキーマになる）  
5. 「Start」実行 → 進捗と結果を確認 → 「Export CSV」で保存
...
スキーマに合わない出力は自動で弾かれ、再試行ポリシーに従って安定化します。
...
詳細は `DEPLOYMENT.md`、アップデート運用は `UPDATES.md` を参照。
